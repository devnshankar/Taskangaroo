...
export default function DashBoard() {
  ...
  return (
    <div className={ `${theme.theme === 'dark' ? "flex border p-4 rounded-lg" : "flex border p-4 rounded-lg"}` }>
      <div
        ...
        <div className="space-y-3">
          <div className={ ` ${open ? "justify-center" : "justify-start"
            ...
            { open ? (<button onClick={ () => setOpen(!open) }>
              ...
              <p className={ `${theme.theme === 'dark' ? "text-md ml pl-2 text-white" : "text-md ml pl-2 text-black"}` }>Dashboard</p>
            </>) }
          </div>
          <Separator />
                ...
                >
                  { open ? (<Home size={ 20 } strokeWidth={ 1.5 } />) : (
                    <>
                      ...
                      <span className={ `${theme.theme === 'dark' ? "text-gray-100" : "text-black"}` }>Home</span>
                    </>
                  ) }
                </Link>
                <Separator />
                                  ...
                                  <CardContent>
                                    <Form { ...form } >
                                      <form onSubmit={ form.handleSubmit(onSubmit) } className="space-y-8">
                                        ...
                                        />
                                        <AlertDialogAction type="submit" className="w-full bg-green-500 text-white font-semibold text-xl">
                                          Create
                                        </AlertDialogAction>
                                        <AlertDialogCancel className="p-5 w-full bg-red-500 rounded-lg text-white font-semibold text-xl">Cancel</AlertDialogCancel>
                                      </form>
                                    </Form>
                                  </CardContent>
                                </Card>
              ...
              <li className="rounded-sm">
                { open ? (<ul className="w-full">
                  ...
                  { categoryNames.map((categoryName, index) => (
                    <li className={ `${theme.theme === 'dark' ? "w-full mb-4 p-2 bg-blue-800 font-semibold text-md  flex items-start space-x-3 rounded-3xl justify-start" : "w-full mb-4 p-2 bg-blue-300 font-semibold text-md  flex items-start space-x-3 rounded-3xl justify-start"}` } key={ index }>
                      <>
                  ...
                  <ul className="w-full">
                    { categoryNames.map((categoryName, index) => (
                      <Button onClick={ () => { setCurrentCategory(categoryName) } } className={ `${theme.theme === 'dark' ? " w-full text-white h-15 mb-3 p-2  align-middle bg-blue-800 font-semibold text-md  flex items-start space-x-3 rounded-3xl justify-start" : " w-full text-black h-15 mb-3 p-2  align-middle bg-blue-300 font-semibold text-md  flex items-start space-x-3 rounded-3xl justify-start"}` } key={ index }>
                        <ClipboardCheck size={ 20 } strokeWidth={ 1.5 } className="mr-1" />
                ...
                <AlertDialog>
                  <AlertDialogTrigger className="w-full">
                    <div
                      ...
                      className={ `${theme.theme === 'dark' ? "w-full bg-red-500 font-bold flex items-center p-2 space-x-3 rounded-md" : "w-full bg-red-500 font-bold flex items-center p-2 space-x-3 rounded-md"}` }
                    >
      ...
      <div className="container mx-auto ">
        ...
        <div className="flex justify-between mx-auto ">
          <span className={ `${theme.theme === 'dark' ? "bg-blue-800 p-2 text-white m-2 rounded-3xl pr-4 pl-4 flex w-auto" : "bg-blue-300 text-black p-2 m-2 rounded-3xl pr-4 pl-4 flex w-auto"}` }>
            <ClipboardCheck size={ 20 } strokeWidth={ 1.5 } className="mr-1" />
            { currentCategory }
          </span>
          <AlertDialog>
        ...
        <Tabs defaultValue="todo" className="w-full pt-4">
          ...
          <TabsContent value="todo">
            <Card>
              <div >
                { (categoryNames.length < 1) ? (<div className='m-10 align-middle flex justify-center text-2xl' >Please Create a Category by clicking on Categories button on side bar</div>) : (<Card className={ `${theme.theme === 'dark' ? "bg-green-900 m-3 mb-0" : "bg-green-300 m-3 mb-0"}` }>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className={ `${theme.theme === 'dark' ? "text-lg font-bold text-white" : "text-lg font-bold text-black"}` }>
                  ...
                  <CardContent>
                    ...
                    { (currentCategory === "") ? (<><p>Please choose one category</p><br /><p>If not any then click on Categories in sidebar to create one</p></>) : (<Link to={ '/task' }>
                      <Button className={ `${theme.theme === 'dark' ? 'bg-green-500 text-white font-semibold text-lg' : 'bg-green-500 text-white font-semibold text-lg'}` } >Create Task</Button> </Link>) }
                ...
                { (!selectedCategory) ? (
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 p-3">
                    { selectedCategory.tasks
                      ...
                      .map(task => (
                        // @ts-expect-error type errors to be resolved
                        <Card className={ `${theme.theme === 'dark' ? 'bg-blue-950' : 'bg-blue-200'}` } key={ task.id }>
                          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                            {/* @ts-expect-error type errors to be resolved */ }
                            <CardTitle className={ `${theme.theme === 'dark' ? 'text-lg font-bold text-blue-500' : 'text-lg font-bold text-blue-600'}` }>{ task.title }</CardTitle>
                            <ListTodo size={ 23 } strokeWidth={ 1.5 } />
                          </CardHeader>
                          <CardContent>
                            <div className="text-sm mb-3">
                              {/* @ts-expect-error type errors to be resolved */ }
                              <p className={ `${theme.theme === 'dark' ? 'text-white' : 'text-black'}` }>{ task.description }</p>
                              {/* @ts-expect-error type errors to be resolved */ }
                              <p className="text-red-400">Deadline: { task.deadline.substring(0, 10) }</p>
                              {/* <p>Status: { task.status }</p> */ }
                              {/* @ts-expect-error type errors to be resolved */ }
                              <p className={ `${theme.theme === 'dark' ? 'text-white' : 'text-black'}` }>Priority: { task.priority }</p>
                            </div>
                            <div>
                              {/* @ts-expect-error type errors to be resolved */ }
              ...
              { (!selectedCategory) ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 p-3">
                  { selectedCategory.tasks
                    ...
                    .map(task => (
                      // @ts-expect-error type errors to be resolved
                      <Card className={ `${theme.theme === 'dark' ? 'bg-yellow-950' : 'bg-yellow-200'}` } key={ task.id }>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                          {/* @ts-expect-error type errors to be resolved */ }
                          <CardTitle className={ `${theme.theme === 'dark' ? 'text-lg font-bold text-blue-500' : 'text-lg font-bold text-blue-600'}` }>{ task.title }</CardTitle>
                          <ListTodo size={ 23 } strokeWidth={ 1.5 } />
              ...
              { (!selectedCategory) ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 p-3">
                  { selectedCategory.tasks
                    ...
                    .map(task => (
                      // @ts-expect-error type errors to be resolved
                      <Card className={ `${theme.theme === 'dark' ? 'bg-green-950' : 'bg-green-200'}` } key={ task.id }>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                          {/* @ts-expect-error type errors to be resolved */ }
                          <CardTitle className={ `${theme.theme === 'dark' ? 'text-lg font-bold text-blue-500' : 'text-lg font-bold text-blue-600'}` }>{ task.title }</CardTitle>
                          <ListTodo size={ 23 } strokeWidth={ 1.5 } />
